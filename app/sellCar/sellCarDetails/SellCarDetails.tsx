...
import { fetchSellCarBrands, fetchSellCarModels, CreateSellCar } from '@/lib/auth';
...
const SellCarDetails: React.FC = () => {
  ...
  // Fetch brand and model names from API if not in sessionStorage
  useEffect(() => {
    const fetchLabels = async () => {
      if (!carDetails) return;

      // Fetch brand name
      if (carDetails.brand && !displayLabels.brand) {
        try {
      ...
      // Fetch model name
      if (carDetails.brand && carDetails.year && carDetails.model && !displayLabels.model) {
        try {
          const modelsResponse = await fetchSellCarModels(carDetails.brand, carDetails.year);
    ...
    fetchLabels();
  }, [carDetails, displayLabels.brand, displayLabels.model]);
